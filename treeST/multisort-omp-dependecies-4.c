...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length, int depth) {
    if (length < MIN_MERGE_SIZE*2L) {
    	...
    	if (!omp_in_final()) {
	  	...
	  	#pragma omp task final (depth >= CUTOFF) depend(in: left[0], right[0]) depend(out: result[0])
		merge(n, left, right, result, start, length/2, depth+1);
		#pragma omp task final (depth >= CUTOFF) depend(in: left[length/2], right[length/2]) depend(out: result[length/2])
...
void multisort(long n, T data[n], T tmp[n], int depth ) {
    if (n >= MIN_SORT_SIZE*4L) {
    	if (!omp_in_final()) {
		// Recursive decomposition
		#pragma omp task final (depth >= CUTOFF) depend(in:tmp[0]) depend(out:data[0])
		multisort(n/4L, &data[0], &tmp[0], depth+1);
		#pragma omp task final (depth >= CUTOFF) depend(in:tmp[n/4L]) depend(out:data[n/4L])
		multisort(n/4L, &data[n/4L], &tmp[n/4L], depth+1);
		#pragma omp task final (depth >= CUTOFF) depend(in:tmp[n/2L]) depend(out:data[n/2L])
		multisort(n/4L, &data[n/2L], &tmp[n/2L], depth+1);
		#pragma omp task final (depth >= CUTOFF) depend(in:tmp[3L*n/4L]) depend(out:data[3L*n/4L])
		multisort(n/4L, &data[3L*n/4L], &tmp[3L*n/4L], depth+1);
		
		
		
		#pragma omp task final (depth >= CUTOFF) depend(in:data[n/4L],data[0]) depend(out:tmp[0])
